...
const MyOrders = () => {
  ...
  const email = (JSON.parse(sessionStorage.getItem("minicred"))).email
  
  const orders = allorders.filter(item=>item.email==email)
        ...
        <tr>  <td colSpan="6"
          ...
          {orders.map((order, index) =>
            <tr key={index}
              ...
              <Link to={`/myorders/${order.id}`} type="button" className='me-2 bg-green-400 text-white p-3 rounded-md '>View</Link>
               </td>
            </tr>
          )}  
        </tbody>
